from util.genoio import load_genotype_from_bedfile
from util.CovConstructor import CovConstructor
import numpy as np
...
def load_mode(mode_list):
    if mode_list[0] not in ['naive', 'cap', 'banded']:
        ...
        if mode_list[0] in ['naive', 'cap']:
            raise ValueError('Currently, we don\'t support naive and cap modes.')
        if len(mode_list) != 2:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', nargs='+', help='''
        Support mode: banded
        Need to set the band size and the result will be saved 
        as sparse npz.
        For instance: --mode banded 200
    ''')
    parser.add_argument('--nbatch', type=int, help='''
        Number of batch to process SNPs.
    ''')
    parser.add_argument('--output_prefix', help='''
        The prefix of output (NPY.GZ).
    ''')
