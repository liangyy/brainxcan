...
sbxcan = read.csv(bxcan_file) %>% mutate(zscore = abs( qnorm(!!sym(bxcan_pval_col)/2) ) * sign(sbxcan$bhat))
allxcan = sbxcan
```
...
The p-value being used is `r pval_col_to_name[[bxcan_pval_col]]`.

Among `r n_pred_idps` IDPs (`r sum(xcandf$modality=="T1")` T1 and `r sum(xcandf$modality=="T1")` diffusion MRI, including the subtype-specific PCs), `r sum(xcandf[[bxcan_pval_col]] < p_thres)` were significantly associated with `r phenotype_name` (pval < $`r signif(p_thres, 3)`$ = 0.05 / `r n_pred_idps`). 
