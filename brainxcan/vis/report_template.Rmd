...
color_map = load_color_code_yaml(color_code)
```
...
xcandf = allxcan_annot %>% filter(subtype %in% names(color_map))
```
...
idp_type = 'T1'
pp = plot_bxcan_ordered(xcandf %>% filter(modality == idp_type), color_map)
...
```{r load_mr}
df_mr = load_mr_sum_stats(input_prefix)
...
```{r dmri_top_n}
xcandf_w_mr %>% select(IDP, modality, subtype, region, zscore, pval, `IDP -> Phenotype`, `Phenotype -> IDP`, link) %>% filter(pval < p_thres) %>% filter(modality=="dMRI") %>% arrange(pval) %>% select(-modality) %>% head(ntop) %>% pander
```

The top `r ntop` associations for T1 are 
```{r t1_top_n, results='asis'}
xcandf_w_mr %>% select(IDP, modality, subtype, region, zscore, pval, `IDP -> Phenotype`, `Phenotype -> IDP`, link) %>% filter(pval < p_thres) %>% filter(modality == "T1") %>% arrange(pval) %>% select(-modality) %>% head(ntop) %>% pander
```
...
for(i in mr_idps$IDP) {
  ...
  fig_fn = Sys.glob(paste0(input_prefix, '*', i, '.MR_vis.pdf'))
  if(length(fig_fn) > 1) {
    next
  }
  info = gannot %>% filter(IDP == i)
  msg = i
  if(nrow(info) == 1) {
    msg = info$notes
  }
